# Cybersecurity Portfolio — Micky Inphantang

Actively seeking **SOC Analyst / GRC** opportunities. This repo documents hands-on projects completed as part of my Cert IV in Cybersecurity — real investigations, real builds, real testing, no invented experience.

## Projects

### 🔎 SOC & Splunk Investigations
Two incident investigations using Splunk SPL, MITRE ATT&CK mapping, and a full incident-response workflow — detection, hypothesis, escalation, and resolution.

**[Incident 1: Brute Force / Username Enumeration Detection](SOC-Splunk-Investigations/incident-01-bruteforce-detection/README.md)**

**[Incident 2: Persistent IKE Handshake Retries (VPN Reconnaissance)](SOC-Splunk-Investigations/incident-02-vpn-ike-retries/README.md)**

### 🛡️ Network Security Lab
A pfSense-based firewall and VPN build, split into two parts.

**[Firewall Configuration](Network-Security-Lab/Firewall-Configuration/README.md)** — zone-based firewall (LAN/DMZ/Internet), NAT, and live rule verification

**[VPN Tunnel](Network-Security-Lab/VPN-Tunnel/README.md)** — site-to-site IPSec VPN and client-to-site OpenVPN, both tested end-to-end

## About
I'm working toward a career in SOC analysis / GRC, currently completing a Cert IV in Cybersecurity. Every project here is my own work, framed honestly as coursework rather than professional experience — I'm not claiming to be more senior than I am, but I wanted to show I can actually do the work, not just talk about it. Every build and investigation in this repo includes verification steps, not just configuration screenshots.

## Skills demonstrated
SIEM investigation (Splunk SPL) & MITRE ATT&CK mapping. Firewall configuration & zone-based security policy (pfSense). VPN architecture (IPSec site-to-site, OpenVPN client-to-site). Network monitoring & SNMP integration. Incident documentation & verification-driven testing.

---
📫 Open to SOC Analyst / GRC opportunities — contact details available on my LinkedIn and CV.
