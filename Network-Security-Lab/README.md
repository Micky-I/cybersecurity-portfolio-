# Network Security Lab: pfSense Firewall with Site-to-Site and Client-to-Site VPN

## Context
Built as part of my Cert IV in Cybersecurity (Network Security unit). The brief was to design, build, and test a firewall solution for a fictional organisation, providing both site-to-site connectivity between two offices and secure remote access for individual users — then document and test every stage.

## Objective
Deploy pfSense as the firewall/VPN gateway for two simulated sites, connect them with an IPSec site-to-site VPN, add a client-to-site OpenVPN option for remote workers, and integrate the whole thing into a monitoring platform — then prove all of it actually works with real connectivity tests rather than just showing configuration screens.
...
The final build had three logical zones: the main LAN (with a client PC and a monitoring server) behind FW-1, a remote office behind FW-2 connected via IPSec, and a remote/roaming worker connecting in over OpenVPN.

![Site-to-site VPN topology between FW-1 and FW-2](screenshots/topology-site-to-site.png)

![Final topology showing the LAN, monitoring server, remote office, and remote worker](screenshots/topology-final.png)

## Build Process

### 1. Base firewall setup
Installed pfSense on FW-1, configured the hostname/domain, and set static IP addressing on the LAN interface. Confirmed the client PC (PC-A) could route out to the internet through the new firewall before moving on to anything else — no point building VPNs on top of a firewall that can't pass basic traffic yet.

![Initial pfSense setup wizard — hostname and domain configuration](screenshots/fw1-initial-setup.png)

### 2. Monitoring integration
Deployed a monitoring VM running Observium, enabled SNMP on FW-1, and added the firewall as a monitored device using SNMP and syslog. This let me confirm the monitoring platform was correctly polling the firewall's status, memory, uptime, and logs in real time — useful for demonstrating that a deployed firewall isn't just "set and forget," it needs to report into the org's existing monitoring stack.
...
### 4. Client-to-site remote access VPN
...
- Observium (SNMP/syslog network monitoring)
